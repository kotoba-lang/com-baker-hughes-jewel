(ns baker-hughes-jewel.main-test
  "Contract + behavioral test for the baker_hughes_jewel-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "baker_hughes_jewel-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'baker-hughes-jewel.main-test)]
