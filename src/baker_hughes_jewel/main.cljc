(ns baker-hughes-jewel.main
  "Kotodama WASM entrypoint for the baker_hughes_jewel clean-room actor (L4) — Clojure port.
  ...
  facts (`baker_hughes_jewel.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "baker_hughes_jewel")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"baker_hughes_jewel.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "baker_hughes_jewel-compat" :tier tier :entities entities} 200])
...
(defn start! [] :baker_hughes_jewel-compat/ready)
